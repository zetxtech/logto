const tenants = {
  ...
  storage: {
    title: '存储服务',
    description: '配置自定义 UI 资源的存储服务（自带 UI）。上传自定义登录体验时需要配置此项。',
    provider: '服务商',
    status_configured: '已配置（{{provider}}）',
    status_not_configured: '未配置。请配置存储服务以启用自定义 UI 上传功能。',
    s3_endpoint: 'S3 端点',
    s3_region: '区域',
    s3_bucket: '存储桶',
    s3_access_key_id: '访问密钥 ID',
    s3_secret_access_key: '访问密钥',
    s3_public_url: '公开 URL（可选）',
    s3_force_path_style: '强制路径样式',
    s3_force_path_style_description:
      '为 MinIO 或 S3 兼容存储启用此选项。使用路径样式 URL（endpoint/bucket）而非虚拟托管样式（bucket.endpoint）。',
    azure_connection_string: '连接字符串',
    azure_container: '容器',
    azure_public_url: '公开 URL（可选）',
  },
  full_env_tag: {
    development: '开发',
    production: '产品',
